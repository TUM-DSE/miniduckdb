...
ROOT = os.path.dirname(HERE)          # this DuckDB checkout
# Paths go out as $(duckdb-dir)/... rather than a fixed prefix: miniosv.mk
# derives that variable from its own location, so the same generated list
# works whether this tree sits under the miniOSv app/ directory or beside it.
PREFIX = "$(duckdb-dir)"
...
def main():
    ...
    for d in include_dirs:
        lines.append(f"duckdb-include-dirs += {PREFIX}/{d}")
    lines.append("")
    ...
    for title, srcs in groups:
        ...
        for s in srcs:
            lines.append(f"duckdb-sources += {PREFIX}/{s}")
        lines.append("")
        total += len(srcs)
